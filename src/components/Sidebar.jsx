...
export default function Sidebar({ isOpen, onClose }) {
  return (
    <div
      className={`fixed top-16 bottom-0 left-0 w-64 bg-[#0C1320] text-white flex flex-col p-4 z-40 transform transition-transform duration-300
  ...
  md:translate-x-0 md:static md:flex`}
    >
      ...
      {/* Perfil: solo se muestra en md o mayores */}
      <div className="hidden md:flex items-center gap-3 justify-center bg-[#06090f] p-3 rounded mb-6">
        <FaUserCircle size={50} className="text-blue-400" />
        <h1 className="text-xl font-semibold">ADMINISTRADOR</h1>
      </div>

      {/* Navegación */}
      <nav className="flex-1 flex flex-col gap-4 p-5 bg-[#1a253f] rounded overflow-y-auto">
